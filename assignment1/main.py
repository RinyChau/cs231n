...
for k in k_choices:
    ...
    for i in xrange(num_folds):
        ...
        for j in xrange(num_folds):
            # if i == j:
            #    continue
            X_train_tmp.extend(X_train_folds[j])
            y_train_tmp.extend(y_train_folds[j])
        X_train_tmp = np.array(X_train_tmp)
        y_train_tmp = np.array(y_train_tmp)
        classifier.train(X_train_tmp,y_train_tmp)
        dists_three = classifier.compute_distances_no_loops(X_test)
        y_test_pred = classifier.predict_labels(dists_three, k=k)
        num_correct = np.sum(y_test_pred == y_test)
        accuracy = float(num_correct) / num_test
        print num_correct
